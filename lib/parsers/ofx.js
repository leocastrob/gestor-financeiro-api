...
async function parsearOFX(buffer) {
    ...
    // Se o banco usa ISO-8859-1/latin1 e lermos como UTF-8, os acentos viram ""
    if (texto.includes('')) {
    ...
    // Prefere o nome do banco emissor (<FI><ORG>) quando o arquivo o expõe;
    // cai para o código do banco (BANKID/ACCTID) se o campo não existir.
    const nomeBanco = dados?.OFX?.SIGNONMSGSRSV1?.SONRS?.FI?.ORG ??
        stmtTrnRs?.STMTRS?.BANKACCTFROM?.BANKID ??
